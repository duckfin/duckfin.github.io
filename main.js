...
document.getElementById('woodText').innerHTML = "Wood: " + (Math.floor(resources['wood']*10)/10).toString()+"/"+Math.floor(resources['woodMax']).toString()+'&nbsp;('+(Math.floor(characters.reduce(function(p,n){return p+((n.assign==assignments["wood"])?n.assign.calcTickGain(n):0);},0)*40)/10).toString()+'/sec)';
